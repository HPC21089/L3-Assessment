...
def ui_info_text():
    ...
    #Enemies left
    draw_text("Enemies Left: " + str(wave.enemies_left), MEDIUM, WHITE, 545, 687)
...
def tutorial():
    ...
    draw_text("Press Z to execute them, or X to clear your current inputs", SMALL, WHITE, 25, 95)
...
def start_screen(arrow_limit):
    ...
    draw_text("X to uhhh, do the opposite or whatever", SMALL, WHITE, 483, 680)
...
def main_menu(arrow_limit, countdown, last_update_time, BID_num):
    ...
    if main_menu_level == "base":
        ...
        draw_text(f"Enemies Defeated: {enemies_defeated}", BIG, WHITE, 472, 680)
    ...
    if main_menu_level == 'moves':
        ...
        draw_text(f"Damage: {COMBOS['combo6']['damage']}", SMALL, WHITE, 530, 330)
        ...
        draw_text(f"Damage: {COMBOS['combo6']['damage']}", SMALL, WHITE, 530, 430)
        ...
        draw_text(f"Damage: {COMBOS['combo7']['damage']}", SMALL, WHITE, 530, 530)
...
win_screen = pygame.transform.scale_by(pygame.image.load('images/Win Screen.png'), 5)
                        ...
                        if event.key == pygame.K_z:
                            ...
                            if output_combo == 'ultimate combo':
                                ...
                                if output_combo == 'failed combo':
                                    ...
                                    cooldown = True
                                    cooldown_charge = 100
                                    if wave_num == 4 and wave.enemies_left == 1:
                                        keys = pygame.key.get_pressed()
                                        z_was_down_before_win = keys[pygame.K_z]
                                        ignore_next_z_keyup = z_was_down_before_win
